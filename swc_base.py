...
class swcFiber:
    def __init__(self, fiber_number, depth):
        self.l = 0
        ...
        self.sum_r = []
        self.pruned = False
        self.dist2soma_head = -1
        self.dist2soma_max = -1
        self.pf = 0
        self.sf = []
        self.al = 0
        self.s_al = 0
    ...
    #
    # def UpdateF(self, point_l, f):
    #     for p in self.p:
    #         if(p not in f.p):
    #             point_l[p].pruned = True
    #     self.p = f.p
    #     self.l = f.l
    #     return point_l

    def UpdateParm(self, point_l):
        if(self.pruned):
            self.l = 0
        else:
            self.UpdateL()
            self.UpdateAvgR(point_l)
            self.UpdateDist2Soma(point_l)
            self.UpdateActLength(point_l)
        if(not self.s_l):self.s_l = self.l
        if(not self.s_al):self.s_al = self.al
        self.Updatei(point_l)
        self.Updater(point_l)
        self.Updatev(point_l)

    def Updatei(self, point_l):
        temp_i = []
        for p in self.p:
            temp_i.append(point_l.p[p].i)
        if(temp_i):
            self.i_mean = np.mean(temp_i)
            self.i_std = np.std(temp_i)

    def Updater(self, point_l):
        temp_r = []
        for p in self.p:
            temp_r.append(point_l.p[p].r)
        if(temp_r):
            self.r_mean = np.mean(temp_r)
            self.r_std = np.std(temp_r)

    def Updatev(self, point_l):
        temp_vector = np.array([0, 0, 0])
        for p in self.p:
            temp_vector = temp_vector + np.array([point_l.p[p].x, point_l.p[p].y, point_l.p[p].z])\
                          - np.array([point_l.p[point_l.p[self.p[0]].p].x,
                                      point_l.p[point_l.p[self.p[0]].p].y,
                                      point_l.p[point_l.p[self.p[0]].p].z])
        self.vector = list(temp_vector)
    def UpdateL(self):
        self.l = len(self.p)

    def UpdateActLength(self, point_l):
        temp_al = 0
        if(not self.p[0] == 1):
            temp_al = CalcswcPointDist(point_l.p[self.p[0]], point_l.p[point_l.p[self.p[0]].p])
        for i in range(self.l - 1):
            temp_al = temp_al + CalcswcPointDist(point_l.p[self.p[i]], point_l.p[self.p[i + 1]])
        self.al = temp_al

    def UpdateDist2Soma(self, point_l):
        self.dist2soma_head = CalcswcPointDist(point_l.p[self.p[0]], point_l.p[1])
        self.dist2soma_max = self.dist2soma_head
        for p in self.p:
            self.dist2soma_max = max(self.dist2soma_max, CalcswcPointDist(point_l.p[p], point_l.p[1]))

    def UpdateAvgR(self, point_l):
        sum_r = 0
        for p in self.p:
            sum_r = sum_r + point_l.p[p].r
            self.sum_r.append(sum_r)
        self.avg_r = sum_r / self.l
